...
function getStr(indx){
	...
	str += "			<input type='text' name='itemName"+indx+"' value='' style='width:150'>							";
	//str += "			<input type='button' value='삭제' onclick=\"delItem('"+indx+"')\">								";
	str += "			<input type='button' value='선택' onclick=\"showItem('"+indx+"')\"></td>							";
	str += "		<td width='200'><input type='text' name='itemDim"+indx+"' value='' style='width:199'></td>			";
	str += "		<td width='90'>																						";
	str += "		  <input type='text' name='itemCnt"+indx+"' value='1' size=4 maxlength=10 onkeyup='calc()'></td>	";
	str += "		<td width='110'>																					";
	str += "		  <input type='text' name='itemPrice"+indx+"' value='0' size=10 maxlength=10 onkeyup='calc()'></td>	";
	str += "		<td width='100'><input type='text' name='totPrice"+indx+"' value='0' size=12 maxlength=14></td>		";
	str += "</tr>																										";
	str += "</table>																									";
	return str;
}
...
%>
	...
	<textarea name='itemName<%=rowIndex+i+1%>' style='display:none'><%=KUtil.nchk(im.itemCom)%> / <%=KUtil.nchk(im.itemName)%></textarea>	
	<textarea name='itemDim<%=rowIndex+i+1%>' style='display:none'><%=KUtil.nchk(im.itemDim)%></textarea>		
