...
function opWinOutExcel(){
	var frm = document.poListForm;
	//var pop = window.open("","viewExcel","width=1,height=1;");
	//frm.target = "viewExcel";
...
<tr align=center height=20 bgcolor="#B3BFD9">
	...
	<td rowspan=2 colspan=7>
		<table cellpadding="0" cellspacing="0" border="0" width="100%" height="100%">
		<tr align=center>
			<td class="menu1" rowspan=2 width="4%">보</td>
			<td class="menu1" rowspan=2 width="17%">매출처</td>
			<td class="menu1" rowspan=2 width="23%">품명 [수량]</td>
			<td class="menu1" width="14%">설치장소</td>
			<td class="menu1" width="14%"><A HREF="javascript:sortby('3')">EXW</A></td>
			<td class="menu1" width="14%"><A HREF="javascript:sortby('4')">ETD</A></td>
			<td class="menu1" width="14%"><A HREF="javascript:sortby('5')">ETA</A></td>
		</tr>
		<tr align=center>
			<td class="menu1"><A HREF="javascript:sortby('6')">납기일</A></td>
			<td class="menu1"><A HREF="javascript:sortby('7')">통관일</A></td>
			<td class="menu1"><A HREF="javascript:sortby('8')">납품일</A></td>
			<td class="menu1">결재방법</td>
		</tr>
		</table></td>
</tr>
...
<tr align=center height=20>
	...
	<td class="<%=clas%>" rowspan=2>
		<A HREF="javascript:opWin('<%=po.seq%>');">
			<span <%=po.eff<0?"class='cline'":""%>><%=poDAO.getPoNo(po)%></span></A></td>
	...
	<td rowspan=2 colspan=7>
		...
		<tr align=center>
			<td width="4%" rowspan=2 class="<%=clas%>" >
				<%=KUtil.nchk(pil.isDropGuar,"N")%></td>
			<td width="17%" rowspan=2 class="<%=clas%>">
				<%	if( ct1==null ) 
						out.print("&nbsp;");
					else
						out.print( KUtil.nchk(ct1.bizName,"&nbsp;") );		%></td>
			<td width="23%" class="<%=clas%>" rowspan=2 style="cursor:hand" onclick="opWinPass1('<%=po.seq%>')" align=left valign=top>
			...
			<%	Vector vecSetupItem = siDAO.getListIn(pil.seq_setupitem);
				...
				<tr>
					<td><A onmouseover="showboxPosition('id_<%=i%><%=h%><%=g%>','block',0,-15);" onmouseout="hide('id_<%=i%><%=h%><%=g%>','none')">&nbsp;
						<%=KUtil.cutTitle( KUtil.nchk(pi.itemName), 13, ".." )%> 
						<FONT COLOR="#3300CC">[<%=KUtil.intToCom(si.cnt)%>]</FONT></A></td>
				</tr>
				</table>
			...
			<td width="14%" class="<%=clas%>" style="cursor:hand" onclick="opWinPass(<%=po.seq%>, '<%=pil.seq%>')">
		...
		<tr align=center>
			<td class="<%=clas%>" style="cursor:hand" onclick="opWinPass(<%=po.seq%>, '<%=pil.seq%>')">
...
<tr align=center height=20>
	<td class="<%=clas%>"><%=po.priceKinds%> <%=NumUtil.numToFmt(po.poTotPrice,"###,###.##","0")%>&nbsp;</td>
</tr>
...
<tr height=50 class="bgc4" align=center>
	<td colspan=11>PO 데이타가 존재하지 않습니다.</td>
</tr>
<%	}	%>
