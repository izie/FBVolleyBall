...
<tr align=center height=20 bgcolor="#B3BFD9">
	<td class="menu1_2" width="50" rowspan=2><A HREF="javascript:sortby('1')"><B>Date</B></A></td>
	<td class="menu1_1" width="75" rowspan=2><A HREF="javascript:sortby('2')">P/O<br>NO</A></td>
	<td class="menu1_1" width="45" rowspan=2>계약<br>NO</td>
	<td class="menu1_1" width="110">PO 업체</td>
	<td rowspan=2 colspan=11>
		<table cellpadding="0" cellspacing="0" border="0" width="100%" height="100%">
		<tr align=center>
			<td class="menu1_1" rowspan=2 width="3%">보</td>
			<td class="menu1_1" rowspan=2 width="10%">매출처</td>
			<td class="menu1_1" rowspan=2 width="14%">품명 [수량]</td>
			<td class="menu1_1" rowspan=2 width="9%">설치장소</td>
			<td class="menu1_1" rowspan=2 width="9%"><A HREF="javascript:sortby('6')">납기일</A></td>
			<td class="menu1_1" rowspan=2 width="9%"><A HREF="javascript:sortby('3')">EXW</A></td>
			<td class="menu1_1" rowspan=2 width="9%"><A HREF="javascript:sortby('4')">ETD</A></td>
			
			<td class="menu1_1" rowspan=2 width="9%"><A HREF="javascript:sortby('5')">ETA</A></td>
			<td class="menu1_1" rowspan=2 width="9%"><A HREF="javascript:sortby('7')">통관일</A></td>
			<td class="menu1_1" rowspan=2 width="9%"><A HREF="javascript:sortby('8')">납품일</A></td>
			<td class="menu1_1" rowspan=2 width="10%">결재방법</td>
		</tr>
		</table></td>
</tr>
<tr align=center height=20 bgcolor="#B3BFD9">
	<td class="menu1_3">금액</td>
</tr>
...
<%	for( int i=vecPo.size()-1 ; i>=0 ; i-- ){	
		...
		String clas		= i%2==0?"m2_1":"m2";													
		String clas1	= i%2==0?"m3_1":"m3";
		int msglen = KUtil.nchk(po.eventMsg).length();	%>

<tr align=center height=20>
	<td class="<%=oby.indexOf("p.wDate") > -1?"m3_s":clas1%>" rowspan=2>
	...
	<td class="<%=oby.indexOf("p.poYear") > -1?"m2_s":clas%>" rowspan=2>
		<A HREF="javascript:opWin('<%=po.seq%>');"><span <%=po.eff<0?"class='cline'":"class='tBold'"%>><%=poDAO.getPoNo(po)%></span></A></td>
	...
	<%	if( po.eff < 0 ){	%>
		<tr align=center>
			<td class="<%=clas%>" style="cursor:hand">
				<B><FONT COLOR="red" size="3">CANCEL</FONT></B></td>
		</tr>
	...
	<%	}else{	%>
		...
		<tr align=center>
			...
			<td width="9%" class="<%=oby.indexOf("s.currDate") > -1?"m2_s":clas%>" style="cursor:hand" onclick="opWinPass(<%=po.seq%>, '<%=pil.seq%>')">
				<%=KUtil.dateMode(pil.currDate,"yyyyMMdd","yy.MM.dd",KUtil.cutTitle(pil.currDateMemo,8,"..","&nbsp;") )%></td>

			<td width="9%" class="<%=oby.indexOf("s.exw") > -1?"m2_s":clas%>" style="cursor:hand" onclick="opWinPass(<%=po.seq%>, '<%=pil.seq%>')">
				<%	ps = psDAO.selectLastOne(pil.seq,"EXW");
					out.println( psDAO.getString(ps, i, h, "&nbsp;") );	%></td>
			<td width="9%" class="<%=oby.indexOf("s.etd") > -1?"m2_s":clas%>" style="cursor:hand" onclick="opWinPass(<%=po.seq%>, '<%=pil.seq%>')">
				<%	ps = psDAO.selectLastOne(pil.seq,"ETD");
					out.println( psDAO.getString(ps, i, h, "&nbsp;") );		%></td>


			<td width="9%" class="<%=oby.indexOf("s.eta") > -1?"m2_s":clas%>" style="cursor:hand" onclick="opWinPass(<%=po.seq%>, '<%=pil.seq%>')">
				<%	ps = psDAO.selectLastOne(pil.seq,"ETA");
					out.println( psDAO.getString(ps, i, h, "&nbsp;") );		%></td>
		
			<td width="9%" class="<%=oby.indexOf("s.pass") > -1?"m2_s":clas%>" style="cursor:hand" onclick="opWinPass(<%=po.seq%>, '<%=pil.seq%>')">
				<%	ps = psDAO.selectLastOne(pil.seq,"통관일");
					out.println( psDAO.getString(ps, i, h, "&nbsp;") );		%></td>
			<td width="9%" class="<%=oby.indexOf("s.deliDate") > -1?"m2_s":clas%>" style="cursor:hand" onclick="opWinPass(<%=po.seq%>, '<%=pil.seq%>')">
