...
try{ 
	...
	int totCnt = cDAO.getTotal(sk, st, "매입");
	...
	Vector vecClient = cDAO.getList(start, pageSize, sk, st, "매입");
...
function sel_item( item_num,seq_client ){
	...
	ofrm.seq_client.value=seq_client;
	ofrm.client_name.value=eval("document.clientForm.client_name"+item_num).value;
...
function move(pge){
	document.location = "fm0_client_list.jsp?nowPage="+pge;
}
function search(){
	var frm = document.clientForm;
