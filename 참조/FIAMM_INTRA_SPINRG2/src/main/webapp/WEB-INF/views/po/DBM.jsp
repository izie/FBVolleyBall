...
try{ 
	...
	po.seq				= KUtil.nchkToInt(request.getParameter("seq_po"));
	...
	System.out.println("$$$$$$$$$$$"+request.getParameter("poKind"));
	

	int inst = poDAO.update(po,saveDir);
	if( inst == -101 ){
		KUtil.scriptAlertBack(out,"다음을 참조하는 정보가 존재합니다.");
		return;
	}else if( inst != 1 ){ 
		KUtil.scriptAlertBack(out,"수정 에러"); 
		return; 
	}
	...
	if( itemUnit != null ){
		...
		int inst1 = piDAO.update(vecPoItem,po.seq);
		if( inst1 != 1 ){
