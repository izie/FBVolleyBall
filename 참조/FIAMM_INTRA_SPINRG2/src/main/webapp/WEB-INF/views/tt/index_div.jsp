...
function selectPil( obj , i ){
	if( obj.checked ){
		document.frames['id_tt'+i].location = "tt_list.jsp?seq_po=<%=seq_po%>&seq_pil="+obj.value;		
		show('id_ttv'+i);
	}
}
</SCRIPT>
...
<tr id="id_ttv<%=i%>" style="display:<%=pil.seq==seq_pil?"block":"none"%>" class="bgc4">
	<td colspan=3>
		<table cellpadding="0" cellspacing="0" border="0" width="100%">
		<tr height=28>
			<td class="ti2" colspan=3>&nbsp; <B>TT 정보</B></td>
		</tr>
		<tr>
			<td class="tdAllLine">
				<iframe <%=pil.seq==seq_pil?"src='tt_list.jsp?seq_po="+seq_po+"&seq_pil="+pil.seq+"'":""%> name="id_tt<%=i%>" id="id_tt<%=i%>" width="100%" height="200" frameborder="0"></iframe></td>
		</tr>
		</table></td>
</tr> 
<%	}//for	%>
